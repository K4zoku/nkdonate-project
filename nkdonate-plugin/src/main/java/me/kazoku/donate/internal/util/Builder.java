...
import me.kazoku.donate.internal.util.collection.map.CaseInsensitiveStringMap;
...
public class Builder<R, V> {
  private final Map<String, BiFunction<String, R, V>> functionMap = new CaseInsensitiveStringMap<>(new HashMap<>());
  private final Map<String, String> nameMap = new CaseInsensitiveStringMap<>(new HashMap<>());
  ...
  public Optional<V> build(String name, R rawValue) {
    return Optional.ofNullable(nameMap.get(name))
        .map(functionMap::get)
        .map(function -> function.apply(name, rawValue));
  }
  ...
  public Map<String, V> build(Map<String, R> rawMap) {
    Map<String, V> map = new CaseInsensitiveStringMap<>(new HashMap<>());
